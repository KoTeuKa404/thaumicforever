...
public class ProxyInventoryScanning {
    ...
    private static void renderToolTip(GuiScreen gui, ItemStack stack, int x, int y) {
        if (renderToolTipMethod == null) {
            try {
                renderToolTipMethod = ReflectionHelper.findMethod(
                    GuiScreen.class,
                    "renderToolTip",
                    "func_146285_a",
                    ItemStack.class,
        ...
        try {
            renderToolTipMethod.invoke(gui, stack, x, y);
        } catch (IllegalAccessException | InvocationTargetException e) {
            // Ignore tooltip failures to avoid crashing the client.
        }
    }
        ...
        if (event.getGui() instanceof GuiContainer && !(event.getGui() instanceof GuiContainerCreative)) {
            ...
            if (!mouseItem.isEmpty() && mouseItem.getItem() == ItemsTC.thaumometer) {
                if (mouseSlot != null && !mouseSlot.getStack().isEmpty()) {
                    ...
                    renderToolTip(event.getGui(), mouseSlot.getStack(), event.getMouseX(), event.getMouseY());
