...
//    private static final TrueTypeFont font = FontLoader.loadSystemFont("Arial", 12.0F, true);
    ...
    public static void blockHighlight(DrawBlockHighlightEvent event) {
        ...
        if(target != null && target.typeOfHit == RayTraceResult.Type.BLOCK
            ...
            drawTagsOnContainer((float)target.getBlockPos().getX() + (float)dir.getFrontOffsetX() / 2.0F, (float)target.getBlockPos().getY() + (float)dir.getFrontOffsetY() / 2.0F, (float)target.getBlockPos().getZ() + (float)dir.getFrontOffsetZ() / 2.0F, ot, 220, dir, event.getPartialTicks());
        }
...
//                        UtilsFX.bindTexture(TileNodeRenderer.nodetex);
    ...
    public static void drawSpecialBlockoverlay(double x, double y, double z, float partialTicks, int color, float alpha) {
        ...
        for(int side = 0; side < 6; ++side) {
            ...
            GlStateManager.rotate(90.0F, (float)(-dir.getFrontOffsetY()), (float)dir.getFrontOffsetX(), (float)(-dir.getFrontOffsetZ()));
    ...
    public static void drawTagsOnContainer(double x, double y, double z, AspectList tags, int bright, EnumFacing dir, float partialTicks) {
        if(Minecraft.getMinecraft().getRenderViewEntity() instanceof EntityPlayer && tags != null && tags.size() > 0) {
            ...
            for(Aspect tag : tags.getAspects()) {
                ...
                GlStateManager.pushMatrix();
                GlStateManager.disableDepth();
                GlStateManager.translate(-iPX + x + 0.5D + (double)(tagscale * 2.0F * (float)dir.getFrontOffsetX()), -iPY + y - (double)shifty + 0.5D + (double)(tagscale * 2.0F * (float)dir.getFrontOffsetY()), -iPZ + z + 0.5D + (double)(tagscale * 2.0F * (float)dir.getFrontOffsetZ()));
