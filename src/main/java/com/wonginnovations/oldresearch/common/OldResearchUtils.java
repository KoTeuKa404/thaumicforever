...
public class OldResearchUtils {
    ...
    private static boolean consumeOffhand(EntityPlayer player, Item item, int md) {
        if (player.inventory.offHandInventory.get(0).isItemEqual(new ItemStack(item, 1, md))) {
    ...
    public static boolean isPlayerCarryingScribeTools(EntityPlayer player) {
        ...
        for (ItemStack stack : player.inventory.mainInventory) {
            if (!stack.isEmpty() && stack.getItem() instanceof IScribeTools && stack.getItemDamage() < stack.getMaxDamage()) {
                return true;
            }
        }
        return false;
