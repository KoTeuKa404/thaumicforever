        ...
        mainThread.addScheduledTask(new Runnable() {
            public void run() {
                ...
                if (tools == null || tools.isEmpty()) {
                    player.sendMessage(new TextComponentString(TextFormatting.RED + I18n.format("researchnote.missing.tools")));
                    failed = true;
                } else if (!((TileResearchTable) te).canConsumeInkFromTable()) {
                    player.sendMessage(new TextComponentString(TextFormatting.RED + I18n.format("tile.researchtable.noink.0")));
                    player.sendMessage(new TextComponentString(TextFormatting.RED + I18n.format("tile.researchtable.noink.1")));
                    failed = true;
                }
                if (!OldResearchUtils.isPlayerCarrying(player, Items.PAPER)) {
                    player.sendMessage(new TextComponentString(TextFormatting.RED + I18n.format("researchnote.missing.paper")));
                    failed = true;
                }
                ...
                for (Aspect aspect : data.aspects.getAspects()) {
                    if (OldResearch.proxy.playerKnowledge.getAspectPoolFor(player.getGameProfile().getName(), aspect) < 1
                        ...
                        player.sendMessage(new TextComponentString(TextFormatting.RED + I18n.format("tc.research.copy.failure", aspect.getName())));
                        failed = true;
                    }
                }
                if (!failed) {
