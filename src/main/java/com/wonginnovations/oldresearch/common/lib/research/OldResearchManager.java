...
//    }
    ...
    private static AspectList limitAspects(AspectList aspects, Random rand, int max) {
        if (aspects == null || max <= 0 || aspects.size() <= max) {
            return aspects;
        }

        List<Aspect> list = new ArrayList<>(Arrays.asList(aspects.getAspects()));
        Collections.shuffle(list, rand);
        AspectList limited = new AspectList();
        int count = Math.min(max, list.size());
        for (int i = 0; i < count; i++) {
            Aspect aspect = list.get(i);
            limited.add(aspect, aspects.getAmount(aspect));
        }
        return limited;
    }
    ...
    public static boolean givePlayerResearchNote(World world, EntityPlayer player, String key) {
    ...
    public static boolean consumeInkFromPlayer(EntityPlayer player, boolean doit) {
        ...
        if (!offhand.isEmpty() && offhand.getItem() instanceof IScribeTools && offhand.getItemDamage() < offhand.getMaxDamage()) {
            if (doit) {
