...
public class TilePort extends TileEntity implements ITickable,
    ...
    public boolean isPrimaryChargerLink() {
        if (world == null || cachedChargerPos == null) return false;
        long now = world.getTotalWorldTime();
        if (!isChargerActive(now) || !isChargerBlock(cachedChargerPos)) return false;

        double myDist = pos.distanceSq(cachedChargerPos);
        int r = CHARGER_SCAN_RANGE;
        BlockPos.MutableBlockPos scan = new BlockPos.MutableBlockPos();
        for (int dx = -r; dx <= r; dx++) {
            for (int dy = -r; dy <= r; dy++) {
                for (int dz = -r; dz <= r; dz++) {
                    scan.setPos(cachedChargerPos.getX() + dx, cachedChargerPos.getY() + dy, cachedChargerPos.getZ() + dz);
                    TileEntity te = world.getTileEntity(scan);
                    if (!(te instanceof TilePort) || te == this || te.isInvalid()) continue;

                    TilePort other = (TilePort) te;
                    if (!cachedChargerPos.equals(other.getChargerTarget())) continue;
                    if (!other.isChargerActive(now)) continue;

                    double otherDist = other.getPos().distanceSq(cachedChargerPos);
