...
public class TileEntityMatteryDuplicator extends TileEntity
    ...
    private boolean hasEnoughVis(float effectiveVis) {
        ...
        float have = getAvailableAuraVis();
    ...
    private boolean drainVis(float effectiveVis) {
        ...
        float sim = drainAvailableAuraVis(effectiveVis, true);
        if (sim + 0.0001f < effectiveVis) return false;

        drainAvailableAuraVis(effectiveVis, false);
        return true;
    }

    private boolean hasArcaneWorkbenchCharger() {
        return world != null
                && BlocksTC.arcaneWorkbenchCharger != null
                && world.getBlockState(pos.up()).getBlock() == BlocksTC.arcaneWorkbenchCharger;
    }

    private float getAvailableAuraVis() {
        if (world == null || pos == null) return 0f;

        if (!hasArcaneWorkbenchCharger()) {
            return AuraHelper.getVis(world, pos);
        }
        ...
        for (int xx = -1; xx <= 1; xx++) {
            for (int zz = -1; zz <= 1; zz++) {
                AuraChunk ac = AuraHandler.getAuraChunk(dim, sx + xx, sz + zz);
                if (ac != null) total += ac.getVis();
            }
        }
    ...
    private float drainAvailableAuraVis(float amount, boolean simulate) {
        if (world == null || pos == null || amount <= 0.0001f) return 0f;

        if (!hasArcaneWorkbenchCharger()) {
            return AuraHelper.drainVis(world, pos, amount, simulate);
        }

        if (simulate) {
            return Math.min(amount, getAvailableAuraVis());
        }

        float remaining = amount;
        float drainedTotal = 0f;
        float chunkRequest = Math.max(1.0f, amount / 9.0f);
        int attempts = 0;

        while (remaining > 0.0001f && attempts++ < 1000) {
            float drainedThisPass = 0f;

            for (int xx = -1; xx <= 1 && remaining > 0.0001f; xx++) {
                for (int zz = -1; zz <= 1 && remaining > 0.0001f; zz++) {
                    float request = Math.min(chunkRequest, remaining);
                    float drained = AuraHelper.drainVis(world, pos.add(xx * 16, 0, zz * 16), request, simulate);
                    if (drained > 0f) {
                        remaining -= drained;
            ...
            if (drainedThisPass <= 0.0001f) break;
        }
    ...
    private void updateGuiFieldsServer() {
        ...
        float vis = 0f;
        try { vis = getAvailableAuraVis(); } catch (Throwable t) { vis = 0f; }
