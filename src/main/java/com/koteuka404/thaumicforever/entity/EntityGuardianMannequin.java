...
public void setWasActivatedByPedestal(boolean b) { this.wasActivatedByPedestal = b; }
    ...
    private void applyArmorAttributes() {
        ...
        for (EntityEquipmentSlot slot : new EntityEquipmentSlot[] {
            ...
            if (!stack.isEmpty()) {
                armorValue += getArmorValue(stack);
                toughnessValue += getArmorToughness(stack);
            }
        }
    ...
    public boolean attackEntityAsMob(Entity entityIn) {
        ...
        if (entityIn instanceof EntityLivingBase) {
            attackDamage += EnchantmentHelper.getModifierForCreature(heldItem, ((EntityLivingBase) entityIn).getCreatureAttribute());
        }
        ...
        boolean hit = entityIn.attackEntityFrom(DamageSource.causeMobDamage(this), attackDamage);
        if (hit) {
            if (knockback > 0) {
                entityIn.addVelocity(
                    -MathHelper.sin(this.rotationYaw * 0.017453292F) * knockback * 0.5F,
                    0.1D,
                    MathHelper.cos(this.rotationYaw * 0.017453292F) * knockback * 0.5F
                );
                this.motionX *= 0.6D;
                this.motionZ *= 0.6D;
            }
            ...
            if (entityIn instanceof EntityLivingBase) {
                ...
                heldItem.getItem().hitEntity(heldItem, (EntityLivingBase) entityIn, this);
                if (heldItem.isEmpty()) {
