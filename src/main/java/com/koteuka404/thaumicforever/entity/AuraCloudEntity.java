...
public class AuraCloudEntity extends Entity {
    ...
    public AuraCloudEntity(World world) {
        super(world);
        // Keep a near-zero physical footprint so block placement is not blocked.
    ...
    public boolean processInitialInteract(EntityPlayer player, EnumHand hand) {
        ...
        return tryUsePhial(player, hand, held) || super.processInitialInteract(player, hand);
    }

    public boolean tryUsePhial(EntityPlayer player, EnumHand hand, ItemStack held) {
        if (world.isRemote) return false;
        if (ThaumcraftApi.internalMethods instanceof DummyInternalMethodHandler) return true;
    ...
    public boolean isInRangeToRenderDist(double distance) {
        // Keep stable render distance even with near-zero hitbox size.
        final double max = 128.0D;
        return distance < max * max;
    }

    @Override
    public AxisAlignedBB getRenderBoundingBox() {
        // Large render AABB prevents culling when node is embedded in blocks.
        return new AxisAlignedBB(
            this.posX - 2.0D, this.posY - 2.0D, this.posZ - 2.0D,
            this.posX + 2.0D, this.posY + 2.0D, this.posZ + 2.0D
        );
    }
    ...
    public void applyEntityCollision(Entity entityIn) {
        // Intentionally empty: this node should not physically collide/push.
    }
