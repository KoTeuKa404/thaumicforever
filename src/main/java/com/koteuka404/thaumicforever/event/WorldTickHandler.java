...
public class WorldTickHandler {
    ...
    public void onWorldTick(TickEvent.WorldTickEvent event) {
        ...
        for (BoundingBox box : boxes) {
            ...
            int maxTotalPerBox = Math.max(1, ModConfig.mazeMaxAliveSkeletonsPerBox);
            int totalNow = st.totalNow;
            if (totalNow >= maxTotalPerBox) continue;

            int maxTotalSpawns = Math.max(0, ModConfig.mazeMaxTotalSkeletonSpawnsPerBox);
            if (maxTotalSpawns > 0 && st.totalSpawned >= maxTotalSpawns) continue;

            int spawnBudget = Math.min(MAX_SPAWNS_PER_TICK_PER_BOX, maxTotalPerBox - totalNow);
            if (maxTotalSpawns > 0) {
                spawnBudget = Math.min(spawnBudget, maxTotalSpawns - st.totalSpawned);
            }
            ...
            if (spawnBudget > 0 && st.angryNow < MAX_ANGRY_PER_BOX) {
                ...
                if (pos != null && spawnAngrySkeletonAt(world, pos)) {
                    st.angryNow++; st.totalNow++; st.totalSpawned++; spawnBudget--;
                }
            }
            ...
            if (spawnBudget > 0 && st.isWaveReady()) {
                ...
                if (waveSlots > 0) {
                    ...
                    st.reviveNow += spawned; st.totalNow += spawned; st.totalSpawned += spawned; spawnBudget -= spawned;
                }
