...
public class ItemScroll extends Item {
   ...
   private void unlockResearch(ItemStack stack, EntityPlayer player) {
    ...
    @SideOnly(Side.CLIENT)
    private String localizeResearchName(String raw) {
        if (raw == null || raw.isEmpty()) {
            return "";
        }

        // Case 1: NBT already stores localization key like "research.X.title"
        if (raw.startsWith("research.") && raw.endsWith(".title")) {
            String translated = I18n.format(raw);
            return translated.equals(raw) ? raw : translated;
        }

        // Case 2: NBT stores research key like "X" -> try "research.X.title"
        String guessedKey = "research." + raw + ".title";
        String translatedGuessed = I18n.format(guessedKey);
        if (!translatedGuessed.equals(guessedKey)) {
            return translatedGuessed;
        }

        // Case 3: fallback to provided text
        String translatedRaw = I18n.format(raw);
        return translatedRaw.equals(raw) ? raw : translatedRaw;
    }
}
