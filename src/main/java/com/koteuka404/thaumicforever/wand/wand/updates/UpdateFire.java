...
public class UpdateFire implements IWandUpdate {
    ...
    public void onUpdate(ItemStack stack, EntityPlayer player) {
        if (player.getEntityWorld().getTotalWorldTime() % 20 == 0) {
            ...
            if (BiomeDictionary.getTypes(b).contains(Type.DRY) && !BiomeDictionary.getTypes(b).contains(Type.NETHER) && WandHelper.getPrimalChargePercentage(stack, player) < 0.5F)
