...
public class UpdateChorusPolymancy implements IWandUpdate {

    @Override
    public void onUpdate(ItemStack stack, EntityPlayer player) {
        if (player.ticksExisted % 20 != 0) return;

        boolean inEnd = player.world.provider.getDimension() == 1;
        float target = inEnd ? 0.90F : 0.45F;
        int recharge = inEnd ? 2 : 1;
