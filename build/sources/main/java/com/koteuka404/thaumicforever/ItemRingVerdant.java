...
public class ItemRingVerdant extends Item implements IBauble {
    ...
    public void onWornTick(ItemStack itemstack, EntityLivingBase entity) {
        if (entity instanceof EntityPlayer) {
            ...
            if (hadBadEffects) {
                player.getCooldownTracker().setCooldown(this, COOLDOWN_TICKS); // Запускаємо перезарядку
                player.world.playSound(null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP, SoundCategory.PLAYERS, 0.5F, 1.0F); // Відтворюємо звук з меншою гучністю
            }
        }
    }
