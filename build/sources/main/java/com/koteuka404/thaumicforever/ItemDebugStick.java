...
public class ItemDebugStick extends Item {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos,
        ...
        if (!pos1.containsKey(playerName)) {
            ...
            player.sendMessage(new TextComponentString("§aПозиція 1 встановлена: " + pos));
        } else if (!pos2.containsKey(playerName)) {
            pos2.put(playerName, pos);
            player.sendMessage(new TextComponentString("§bПозиція 2 встановлена: " + pos));
            replaceAirWithStructureVoid(world, playerName);
        } else {
            pos1.remove(playerName);
            pos2.remove(playerName);
            player.sendMessage(new TextComponentString("§cВиділення скинуто."));
        }

        return EnumActionResult.SUCCESS;
    }

    private void replaceAirWithStructureVoid(World world, String playerName) {
        ...
        world.getMinecraftServer().getPlayerList().getPlayerByUsername(playerName)
                .sendMessage(new TextComponentString("§eЗаміна завершена. replaced: " + replaced));
    }
