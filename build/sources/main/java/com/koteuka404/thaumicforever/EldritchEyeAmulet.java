...
public class EldritchEyeAmulet extends Item implements IBauble {
    ...
    public EldritchEyeAmulet() {
        ...
        MinecraftForge.EVENT_BUS.register(this); // Реєстрація обробника подій
    }
    ...
    public void fogSetColor(EntityViewRenderEvent.FogColors event) {
        ...
        // Перевіряємо, чи носить гравець амулет
        if (player != null && isWearingAmulet(player)) {
            event.setRed(0.0F);    // Чорний колір
            event.setGreen(0.0F);
            event.setBlue(0.0F);
        }
    }
    ...
    public void fogDensity(EntityViewRenderEvent.RenderFogEvent event) {
        ...
        if (player != null && isWearingAmulet(player)) {
            GlStateManager.setFog(GlStateManager.FogMode.LINEAR); // Лінійний туман
            GlStateManager.setFogStart(0.0F);
            GlStateManager.setFogEnd(12.0F);   // Відстань туману
            GlStateManager.setFogDensity(0.3F); // Густина туману
        }
    }


    private boolean isWearingAmulet(EntityPlayer player) {
        // Перевіряємо слот для амулету
