...
public class WorldTickHandler {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (isValidSpawnPosition(world, spawnPos)) {
            ...
            // Додаємо предмет у руки з шансом 5%
            if (random.nextInt(100) < 5) { // 5% шанс
                int randomMeta = random.nextInt(4); // Випадкова мета-дана (0-3)
                ItemStack scroll = new ItemStack(ModItems.SCROLL_P, 1, randomMeta); // Створюємо ItemStack
                reviveSkeleton.setHeldItem(EnumHand.MAIN_HAND, scroll); // Додаємо в руку
            }
